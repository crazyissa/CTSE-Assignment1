...
export default function PaymentSuccess() {
  ...
  const handleTrackOrder = () => {
    if (payment?.deliveryId) {
      navigate(`/track/${payment.deliveryId}`); // ✅ uses real MongoDB delivery ID
    } else {
      alert("Delivery ID not found. Please contact support.");
    }
  };
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-red-50">
        <div className="text-lg font-semibold text-red-600">
          {error || "Error: No payment data found!"}
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md text-center">
        ...
        <div className="bg-green-100 p-4 rounded-md mb-6">
          <p className="text-lg text-gray-700">
            Order ID: <strong>{payment.orderId || 'N/A'}</strong>
          </p>
          <p className="text-lg text-gray-700">
            Amount: <strong>LKR {payment.amount}</strong>
          </p>
          <p className="text-lg text-gray-700">
            Status: <strong>{payment.paymentStatus}</strong>
          </p>
        </div>

        <button
          onClick={handleTrackOrder}
          disabled={!payment.deliveryId}
          className="mt-4 bg-green-600 hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white px-6 py-3 rounded-full text-lg font-semibold transition-all duration-300"
        >
          🚚 Track Your Order
        </button>
        ...
        {!payment.deliveryId && (
          <p className="mt-2 text-sm text-red-500">
            Delivery ID missing — tracking unavailable.
          </p>
        )}
      </div>
    </div>
